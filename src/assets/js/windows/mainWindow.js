...
async function createWindow() {
    ...
    notificationWindow.setAlwaysOnTop(true, "floating", 1);
    notificationWindow.setSkipTaskbar(true);

    
          
    notificationWindow.loadFile(path.join(electron.app.getAppPath(), 'src', 'panels', 'music-small.html'))
    ...
    mainWindow.once('ready-to-show', () => {
        mainWindow.webContents.executeJavaScript("localStorage.getItem('lang')").then((result) => {
            lang = result;
            if (lang === null || lang === undefined || lang === "" || lang === "null") {
                selectLangWindow.loadFile(path.join(electron.app.getAppPath(), 'src', 'panels', 'select-lang.html'));
